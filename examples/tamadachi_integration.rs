...
use mali_g68::emulator::{TamadachiMetrics, TamadachiOptimizer};
use mali_g68::mem::pool::PoolManager;
use mali_g68::userspace::{init_user_space_driver, UserSpaceConfig, UserSpaceContext};
use std::sync::Arc;
...
impl TamadachiIntegration {
    ...
    fn display_tamadachi_metrics(&self) {
        ...
        println!(
            "| FPS: {:.1} / 60 (Target)        |",
            tamadachi_metrics.current_fps
        );
        println!(
            "| Frame Time: {:.2}ms               |",
            tamadachi_metrics.avg_frame_time_ms
        );
        println!(
            "| Optimization: {}                 |",
            tamadachi_metrics.current_optimization_level
        );
        println!(
            "| Batch Efficiency: {:.1}%          |",
            tamadachi_metrics.batch_efficiency
        );
        println!(
            "| Texture Hit Rate: {:.1}%          |",
            tamadachi_metrics.texture_cache_hit_rate
        );
        println!(
            "| Shader Hit Rate: {:.1}%           |",
            tamadachi_metrics.shader_cache_hit_rate
        );
        println!(
            "| Memory: {:.1}MB / {}MB            |",
        ...
        println!(
            "| Atlases Active: {}                |",
            tamadachi_metrics.active_atlases
        );
        println!(
            "| Draws Merged: {}                  |",
            tamadachi_metrics.total_draws_merged
        );
        println!("=====================================");
        println!("| Tamadachi Optimizations:                |");
        println!(
            "| - Game Optimizations: {:<5}            |",
            if tamadachi_metrics.game_optimizations_enabled {
                "ON"
            } else {
                "OFF"
            }
        ...
        println!(
            ...
            if tamadachi_metrics.texture_compression_enabled {
                "ON"
            } else {
                "OFF"
            }
        ...
        println!(
            "| - Ultra Batching: {:<5}                  |",
            if tamadachi_metrics.ultra_batching_enabled {
                "ON"
            } else {
                "OFF"
            }
        ...
        println!(
            "| - Est. FPS Gain: +{:.0}%                  |",
            tamadachi_metrics.estimated_fps_gain
        );
        println!("=====================================");
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Game Optimizations: {}",
        if metrics.game_optimizations_enabled {
            "✅ ENABLED"
        } else {
            "❌ DISABLED"
        }
    ...
    println!(
        "   Texture Compression: {}",
        if metrics.texture_compression_enabled {
            "✅ ENABLED"
        } else {
            "❌ DISABLED"
        }
    ...
    println!(
        "   Ultra Batching: {}",
        if metrics.ultra_batching_enabled {
            "✅ ENABLED"
        } else {
            "❌ DISABLED"
        }
